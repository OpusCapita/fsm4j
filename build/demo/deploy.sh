...
display_usage() {
  printf "Usage:\n\n"
  printf "deploy.sh \\ \n"
  printf "  --ci-build-url=http://https://circleci.com/gh/OpusCapita \\ # CI build/deployment process URL\n"
}
...
helm upgrade \
  ...
  --set database.auto.connection.name="${deployment_namespace}" \
  ...
  --set slack-notifications.ci.jobUrl="$ci_build_url" \

...
if git describe --tags --exact-match "$git_revision" &>/dev/null; then
  git_tag=$( generate_docker_tag "$( git describe --tags --exact-match $git_revision )" )
  # remove leading 'v', e.g. v1.0.0 -> 1.0.0
